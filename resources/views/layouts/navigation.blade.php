<nav x-data="{ open: false }" class="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-b border-white/20 dark:border-gray-700/50 h-20 fixed w-full z-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full">
        <div class="flex justify-between h-full">
            ...
            <div class="flex items-center">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="flex items-center group">
                        <div class="relative">
                            <img src="{{ asset('logo-kanboard.png') }}" alt="kanboard-logo" class="h-8 sm:h-12 w-auto object-cover transition-transform duration-300 group-hover:scale-110">
                        ...
                        <span class="ml-2 sm:ml-3 text-lg sm:text-xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Kanboard</span>
                    </a>
                </div>
            ...
            <div class="flex items-center space-x-2 sm:space-x-4">
                ...
                <button class="md:hidden p-2 rounded-xl bg-gray-100/80 dark:bg-gray-800/80 text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-all duration-200">
                    <i class="fas fa-search text-lg"></i>
                </button>

                <!-- Bouton de thème -->
                <button onclick="toggleTheme()" class="p-2 rounded-xl bg-gray-100/80 dark:bg-gray-800/80 text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-all duration-200">
                ...
                <div class="hidden lg:flex items-center">
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center px-3 py-2 rounded-xl bg-gray-100/80 dark:bg-gray-800/80 text-gray-700 dark:text-gray-300 hover:bg-gray-200/80 dark:hover:bg-gray-700/80 transition-all duration-200 font-medium text-sm">
                                <div class="w-6 h-6 xl:w-8 xl:h-8 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold mr-2 xl:mr-3 text-xs xl:text-sm">
                                    {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                                </div>
                                <div class="hidden xl:block">{{ Auth::user()->name }}</div>
                                <div class="ms-1 xl:ms-2">
                                    <svg class="fill-current h-3 w-3 xl:h-4 xl:w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                        ...
                        <x-slot name="content">
                            ...
                            <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
                                <div class="flex items-center sm:block">
                                    ...
                                    <div class="w-10 h-10 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold mr-3 sm:hidden">
                                        {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                                    </div>
                                    <div>
                                        <p class="text-sm sm:text-base text-gray-900 dark:text-white font-medium truncate">{{ Auth::user()->name }}</p>
                ...
                <!-- Hamburger mobile -->
                <div class="lg:hidden">
                    <button @click="open = ! open" class="inline-flex items-center justify-center p-2 rounded-xl bg-gray-100/80 dark:bg-gray-800/80 text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-all duration-200">
        ...
        <div :class="{ 'block': open, 'hidden': !open }" class="lg:hidden absolute top-full left-0 right-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-b border-gray-200 dark:border-gray-700 shadow-xl z-50">
            ...
            <div class="px-4 py-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <div class="space-y-2">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <x-responsive-nav-link :href="route('logout')" onclick="event.preventDefault(); this.closest('form').submit();" class="flex items-center px-4 py-3 rounded-xl text-red-600 hover:text-red-700">
